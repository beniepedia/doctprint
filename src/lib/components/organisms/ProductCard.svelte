...
<a href={`#/product?id=${p.id}`} class={`snap-start ${horizontal ? 'w-[160px] md:w-[220px] shrink-0' : ''} bg-surface rounded-lg border border-border/60 flex flex-col overflow-hidden hover:shadow-[0_4px_6px_-1px_rgba(0,0,0,0.05)] transition-shadow duration-300`} style={`view-transition-name:product-card-${p.id}`}>
