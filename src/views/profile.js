    ...
    return `
      ...
      <main class="w-full max-w-3xl mx-auto px-4 md:px-8 flex flex-col gap-6 pt-20 pb-8">
        ...
        <section class="flex flex-col items-center text-center mt-4">
          ...
          <h1 class="text-2xl font-bold md:text-3xl text-primary">Budi Santoso</h1>
          ...
          ${stats
            .map(
              ...
              <div class="bg-surface border border-border rounded-lg p-4 flex flex-col items-center justify-center shadow-sm hover:shadow-md transition-shadow">
                <span class="material-symbols-outlined text-accent-container text-[32px] mb-2" style="font-variation-settings:'FILL' 1">${s.icon}</span>
                <span class="text-2xl font-semibold text-primary">${s.value}</span>
            ...
            .map(
              ...
              <a href="#" class="flex items-center justify-between p-4 ${i < profileMenu.length - 1 ? 'border-b border-border' : ''} hover:bg-surface-low transition-colors group">
                <div class="flex items-center gap-4">
                  ...
                  <span class="text-lg text-text-primary">${m.label}</span>
                </div>
        ...
        <section class="mt-6 mb-8">
          <button data-action="logout" class="w-full bg-surface border border-error text-error rounded-lg py-4 text-base font-semibold hover:bg-error-container hover:border-error transition-colors flex items-center justify-center gap-2 shadow-sm active:scale-95 duration-150">
